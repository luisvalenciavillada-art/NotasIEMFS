 ...
 */
/** true si la app corre en http en esta máquina o LAN (misma carpeta que servidor_local.py + /notas-gas-proxy). */
...
function aplicarVisibilidadEstadoResumenFaltas() {
  const wrap = document.getElementById("rf-wrap");
  const chk = document.getElementById("rf-col-estado");
  if (!wrap) return;
  const on = chk?.checked === true;
  wrap.dataset.showEstado = on ? "on" : "off";
}

function aplicarVisibilidadNumeroResumenFaltas() {
  const wrap = document.getElementById("rf-wrap");
  const chk = document.getElementById("rf-col-num");
  if (!wrap) return;
  const on = chk?.checked === true;
  wrap.dataset.showNum = on ? "on" : "off";
}
  ...
  return conUnReintentoRed(async () => {
    ...
    if (!res.ok) {
      ...
      if (st === 405) {
        ...
        throw new Error(
          "Error HTTP 405: el puerto no esta sirviendo el proxy de Notas IEMFS (otro programa ocupa el puerto o no uso iniciar-servidor.bat). " +
            "Cierre Live Server / npm run preview:phone, ejecute iniciar-servidor.bat y en el navegador abra " +
            (origin ? `${origin}/notas-iemfs-health` : "/notas-iemfs-health") +
            " — debe verse JSON con server notas-iemfs-local."
        );
      }
...
function renderListaEstudiantes(data) {
  ...
  body.innerHTML = filtrados.map((e) => {
    ...
    const celdaDetalle = `<td class="group-board__detalle"><button type="button" class="btn btn--secondary btn--sm btn-ver-detalle" data-estudiante-id="${escapeHtml(e.id)}" aria-label="Ver ficha del estudiante"><span class="btn-ver-detalle__long">Ver detalle</span><span class="btn-ver-detalle__short" aria-hidden="true">Ficha</span></button></td>`;
    return `
      <tr>
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("rf-col-estado")?.addEventListener("change", () => {
    aplicarVisibilidadEstadoResumenFaltas();
  });
  document.getElementById("rf-col-num")?.addEventListener("change", () => {
    aplicarVisibilidadNumeroResumenFaltas();
  });
